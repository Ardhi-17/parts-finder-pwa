...
function SuggestPageContent() {
  ...
  useEffect(() => {
    setForm({
      equipment_model: searchParams.get("equipment_model") || "",
      service_type: searchParams.get("service_type") || "",
      part_no: searchParams.get("part_no") || "",
      current_description: searchParams.get("description") || "",
    });
  }, [searchParams]);
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-[#F4F6F8] p-4">
          <p className="rounded-2xl bg-white p-4 text-sm font-semibold text-[#0B1E3A] shadow-sm">
            Memuat form saran...
          </p>
        </div>
      }
